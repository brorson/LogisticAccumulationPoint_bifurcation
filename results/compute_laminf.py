...
if (__name__ == "__main__"):
    ...
    print("len(x2) = "+str(len(x2)))
    ax2 = seq.compute_lambdainf(x2)
    M = len(ax2)
    while (M > 2):
        ax2 = seq.compute_lambdainf(ax2)
        M = len(ax2)
    # Now we're done.  Print out result.
    print("Lambda_inf computed from accelerated sequence ax2 = ")
    ...
    seq.compare_lambdas(ax2[-1], seq.lambdainf)
